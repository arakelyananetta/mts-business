...
function CampaignLauncher({ ctx, onClose, initChans, initTab }) {
  const [tab, setTab] = useState(initTab || 'channels')
  const [chans, setChans] = useState(initChans || ['sms'])
              ...
              <div className="chan-grid">
                {CHANNELS.map((c) => (
                  <button key={c.id} className={`chan${chans.includes(c.id) ? ' on' : ''}`} onClick={() => toggleChan(c.id)}>
                    ...
                    <span><b>{c.name}</b><i>{c.hint}{c.price ? ` · ${c.price}` : ''}</i></span>
                  </button>
                ))}
              </div>
            ) : (
